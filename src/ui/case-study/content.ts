...
export const SECTIONS: CaseSection[] = [
  ...
  },
  {
    ...
    body: [
      ...
      '<strong>it tests the core hypothesis instead of assuming it.</strong> the whole design rests on one claim: that the four environments are four views of a single latent constant. so the result fits each faculty\'s <em>own</em> peak and shows whether they actually agree. when they converge, that is the thesis holding up in front of you; when they scatter, the disagreement is shown rather than averaged away. <span class="cs-mark">the honest failure is the point</span> - a fused average that hides four faculties quietly disagreeing would be the more impressive-looking lie.',
  ...
  },
  {
    ...
    body: [
      'the core (<strong>convert · scoring · optimizer · stats</strong>) is plain typescript, tested against published formulas: iso 9241-9 throughput, a constant-velocity kalman filter, a hand-rolled cholesky solve for the gaussian process. the cholesky and the gp are hand-rolled rather than pulled from a library on purpose - a portfolio measurement a reviewer cannot read line by line is a measurement a reviewer cannot trust. no framework, no backend, 500+ passing tests.',
      'the seams are deliberate. every instrument is a pure <code>analyze()</code> plus a thin <code>run()</code> shell, so the math is tested against synthetic players and only the raw pointer/raf glue is runtime-only. the data-viz above is the same idea: a pure <code>plotGeometry()</code> (domain→pixel, fully unit-tested) and a thin renderer that only writes svg attributes. the cosmetic 3d arena is fenced off from the measurement by a test that proves the scored stream is <em>byte-identical</em> with the skin on or off - the graphics can never move the number.',
    ...
    spec: [
      ...
      { k: 'tests', v: '500+, pure core tdd', mono: true },
